...
With the new **Imputation Server 2** pipeline, you can conveniently run your imputation workflow locally using **Nextflow**. This can be done either by setting up a [local imputation server](https://genepi.github.io/michigan-imputationserver/tutorials/local-server/) or, as in this example, by running it directly from the command line.
...
mkdir imputation-locally
cd imputation-locally
# Download Repository
...
```bash
nextflow run imputationserver2/main.nf -c imputation.config -profile docker
```
